...
impl Shrub {
    ...
    pub fn cleaned(&self) -> Self {
        ...
        if serializer_helper::is_default_grow_duration(cleaned.grow_duration) {
            cleaned.grow_duration = None;
        }
        if serializer_helper::is_zero(cleaned.value) {
            cleaned.value = None;
        }
        if serializer_helper::is_default_picked_tile(cleaned.picked_tile) {
            cleaned.picked_tile = None;
        }
        if serializer_helper::is_default_dead_picked_tile(cleaned.dead_picked_tile) {
            cleaned.dead_picked_tile = None;
        }
        if serializer_helper::is_default_shrub_tile(cleaned.shrub_tile) {
            cleaned.shrub_tile = None;
        }
        if serializer_helper::is_default_dead_shrub_tile(cleaned.dead_shrub_tile) {
            cleaned.dead_shrub_tile = None;
        }
        if serializer_helper::is_default_cluster_size(cleaned.cluster_size) {
        ...
        if serializer_helper::is_default_shrub_drown_level(cleaned.shrub_drown_level) {
